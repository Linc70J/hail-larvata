...
<div data-bind="visible: loadingItem" class="loading"><?php echo trans('administrator.loading') ?></div>

<form class="edit_form" data-bind="visible: !loadingItem(), submit: saveItem">
	<h2 data-bind="text: $root[$root.primaryKey]() ? '<?php echo trans('administrator.edit') ?>' :
		'<?php echo trans('administrator.createnew') ?>'"></h2>

	<!-- ko if: $root[$root.primaryKey]() -->
		<!-- ko if: $root.itemLink() -->
			<a class="item_link" target="_blank" data-bind="attr: {href: $root.itemLink()},
							text: '<?php echo trans('administrator.viewitem', array('single' => $config->getOption('single'))) ?>'"></a>
		<!-- /ko -->
	<!-- /ko -->
		...
		<!-- /ko -->
			...
			<!-- ko if: type === 'image' -->
						...
						<div class="uploader" data-bind="attr: {disabled: $root.freezeForm, id: field_name + '_uploader'}, value: $root.activeItem,
												fileupload: {field: field_name, size_limit: size_limit, uploading: uploading, image: true,
															upload_percentage: upload_percentage, upload_url: upload_url}">
																<?php echo trans('administrator.uploadimage') ?></div>
						<!-- ko if: uploading -->
							<div class="uploading"
							data-bind="text: '<?php echo trans('administrator.imageuploading') ?>' + upload_percentage() + '%'"></div>
						<!-- /ko -->
					</div>
				<!-- /ko -->
				...
				<!-- ko if: !$root[field_name]() && !editable -->
					<div class="uneditable" data-bind="text: '<?php echo trans('administrator.no_image_uploaded') ?>'"></div>
				<!-- /ko -->
			<!-- /ko -->
			...
			<!-- ko if: type === 'file' -->
					...
					<div class="upload_container" data-bind="attr: {id: field_id}">
						<div class="uploader" data-bind="attr: {disabled: $root.freezeForm, id: field_name + '_uploader'}, value: $root.activeItem,
												fileupload: {field: field_name, size_limit: size_limit, uploading: uploading,
															upload_percentage: upload_percentage, upload_url: upload_url}">
																<?php echo trans('administrator.uploadfile') ?></div>
						<!-- ko if: uploading -->
							<div class="uploading"
							data-bind="text: '<?php echo trans('administrator.fileuploading') ?>' + upload_percentage() + '%'"></div>
						<!-- /ko -->
					</div>
				<!-- /ko -->
				...
				<!-- /ko -->
				<!-- ko if: !$root[field_name]() && !editable -->
					<div class="uneditable" data-bind="text: '<?php echo trans('administrator.no_file_uploaded') ?>'"></div>
				<!-- /ko -->
			<!-- /ko -->
	...
	<div class="control_buttons">
		<!-- ko if: $root[$root.primaryKey]() -->
			<input type="button" class="btn btn-block btn-default" value="<?php echo trans('administrator.close') ?>"
				data-bind="click: closeItem, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />

			<!-- ko if: actionPermissions.update -->
				<input type="button" class="btn btn-block btn-primary" value="<?php echo trans('administrator.save') ?>"
					data-bind="click: function() { saveItem(true) }, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />
			<!-- /ko -->
		<!-- /ko -->

		<!-- ko ifnot: $root[$root.primaryKey]() -->
			<input type="button" class="btn btn-block btn-default" value="<?php echo trans('administrator.cancel') ?>"
				data-bind="click: closeItem, attr: {disabled: $root.freezeForm() || $root.freezeActions()}" />
			<!-- ko if: actionPermissions.create -->
				<input type="submit" class="btn btn-block btn-primary" value="<?php echo trans('administrator.create') ?>"
